...
perf-lab-data-body = Die zentralen Core Web Vitals (LCP, CLS, TBT) werden mit dem Lighthouse-Mobile-Profil gemessen — näherungsweise eine langsame 4G-Verbindung mit vierfacher CPU-Drosselung — und spiegeln damit ein Mittelklasse-Mobilgerät wider; sie fallen bewusst konservativer aus als ein ungedrosselter Desktop-Aufruf. Eine große Seite kann hier deshalb einen zweistelligen LCP zeigen, den ein schneller Desktop-Browser nicht hätte. Alle Werte sind lokale Headless-Lab-Messungen (Chrome/CDP), keine Felddaten (CrUX/Real-User-Monitoring); die Tabelle der gedrosselten Bedingungen weiter unten listet die übrigen Profile. Mit „Lab-Schätzung" markierte Kennzahlen (INP, TTI, Speed Index) sind abgeleitete Näherungen, keine direkten Messwerte.
