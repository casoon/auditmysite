...
use audit::browser::{find_chrome, BrowserManager, BrowserOptions};
use audit::cli::{Args, OutputFormat};
...
async fn run(args: Args) -> Result<()> {
    ...
    if let Err(e) = args.validate() {
        return Err(audit::error::AuditError::ConfigError(e));
    }
...
async fn run_single_mode(args: &Args) -> Result<()> {
    ...
    if report
        ...
        .violations
        .iter()
        .any(|v| v.severity == audit::Severity::Critical)
    {
        std::process::exit(1);
    }
...
async fn run_batch_mode(args: &Args) -> Result<()> {
    ...
    let urls = if let Some(ref sitemap_url) = args.sitemap {
        ...
        return Err(audit::error::AuditError::ConfigError(
...
fn output_single_report(report: &audit::AuditReport, args: &Args) -> Result<()> {
    match args.format {
        OutputFormat::Json => {
            let json_report =
...
fn output_batch_report(batch_report: &audit::audit::BatchReport, args: &Args) -> Result<()> {
    match args.format {
        OutputFormat::Json => {
            let output = serde_json::to_string_pretty(batch_report).map_err(|e| {
                audit::error::AuditError::OutputError {
                    reason: e.to_string(),
                }
            })?;
...
fn print_batch_table(batch_report: &audit::audit::BatchReport, args: &Args) {
    println!();
...
fn format_batch_markdown(batch_report: &audit::audit::BatchReport) -> String {
...
fn write_output(content: &str, path: &PathBuf) -> Result<()> {
    fs::write(path, content).map_err(|e| audit::error::AuditError::FileError {
        path: path.clone(),
        reason: e.to_string(),
    })
}
...
fn format_markdown_report(report: &audit::AuditReport) -> String {
    let mut output = String::new();
