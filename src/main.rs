...
use auditmysit::audit::{
    parse_sitemap, read_url_file, run_concurrent_batch, run_single_audit, BatchConfig,
    PipelineConfig,
};
use auditmysit::browser::{find_chrome, BrowserManager, BrowserOptions};
use auditmysit::cli::{Args, OutputFormat};
use auditmysit::error::{AuditError, Result};
use auditmysit::output::{
...
async fn run(args: Args) -> Result<()> {
    ...
    if let Err(e) = args.validate() {
        return Err(auditmysit::error::AuditError::ConfigError(e));
    }
...
async fn run_single_mode(args: &Args) -> Result<()> {
    ...
    if report
        ...
        .violations
        .iter()
        .any(|v| v.severity == auditmysit::Severity::Critical)
    {
        std::process::exit(1);
    }
...
async fn run_batch_mode(args: &Args) -> Result<()> {
    ...
    let urls = if let Some(ref sitemap_url) = args.sitemap {
        ...
        return Err(auditmysit::error::AuditError::ConfigError(
...
fn output_single_report(report: &auditmysit::AuditReport, args: &Args) -> Result<()> {
    match args.format {
        OutputFormat::Json => {
            let json_report =
...
fn output_batch_report(batch_report: &auditmysit::audit::BatchReport, args: &Args) -> Result<()> {
    match args.format {
        OutputFormat::Json => {
            let output = serde_json::to_string_pretty(batch_report).map_err(|e| {
                auditmysit::error::AuditError::OutputError {
                    reason: e.to_string(),
                }
            })?;
...
fn print_batch_table(batch_report: &auditmysit::audit::BatchReport, args: &Args) {
    println!();
...
fn format_batch_markdown(batch_report: &auditmysit::audit::BatchReport) -> String {
...
fn write_output(content: &str, path: &PathBuf) -> Result<()> {
    fs::write(path, content).map_err(|e| auditmysit::error::AuditError::FileError {
        path: path.clone(),
        reason: e.to_string(),
    })
}
...
fn format_markdown_report(report: &auditmysit::AuditReport) -> String {
    let mut output = String::new();
