...
const VALID_SITEMAP_URL = 'https://example.com/sitemap.xml';
...
describe('Stable Audit Interface Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    auditor.onProgress((progress) => {
      ...
      console.log(
        `📊 Progress: ${progress.phase} - ${progress.progress}% (${progress.completed}/${progress.total})`
      );
    });
  ...
  describe('🛡️ Error Handling and Recovery Tests', () => {
    ...
    it('should prevent audit without initialization', async () => {
      await expect(auditor.auditWebsite(VALID_SITEMAP_URL)).rejects.toThrow(
    ...
    it('should detect if queue progress gets stuck at 0%', async () => {
      ...
      const progressPromise = new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(
            new Error(
              'Progress appears to be stuck - no progress updates received within 30 seconds'
            )
          );
        }, 30000);
