...
**Kern-Fokus:** Sitemap-Audit mit modularen Einzelaudits
**Priorität 1:** JSON-Export (vollständiger Audit-Report)
**Priorität 2:** API-Module (einzelne Audit-Bereiche für Electron App)
**Priorität 3:** HTML-Report (aktuell wichtigster Nice-to-Have)
**Grundsatz:** Gut wartbar, modular und fehlerfrei
...
**Ziel:** Konsistente, gut strukturierte JSON-Ausgabe (vollständiger Report)
...
**Architektur-Prinzip:** 
- JSON wird IMMER generiert (basierend auf strikten TypeScript Types)
- HTML-Generator liest JSON-Datei und nutzt die gleichen Types
- Strikte 1:1 Type-Kompatibilität zwischen Pipeline, JSON und HTML

#### 2.1 Einheitlicher HTML-Generator
**Entfernen:** Legacy und Enhanced HTML-Generatoren
**Neue Lösung:** JSON-basierte, typisierte Section-Architektur

```typescript
// Strikte Type-Definition (Grundlage für JSON, HTML und API)
...
// Unter-Types (1:1 oder 1:n Beziehung, auch für API einzeln abrufbar)
...
// HTML-Generator nutzt gleiche Types
class UnifiedHTMLGenerator {
  ...
  // Liest JSON-Datei und nutzt strikt typisierte Daten
  async generateFromJSON(jsonPath: string): Promise<string> {
    const data: FullAuditResult = JSON.parse(await fs.readFile(jsonPath, 'utf8'));
...
### Phase 3: API-Modularisierung (Priorität: HOCH)
**Zeitaufwand:** 2-3 Wochen

#### 3.1 Modulare Audit-Services
**Ziel:** Einzelne Audit-Bereiche nutzen die gleichen Types wie JSON/HTML
**Prinzip:** API nutzt Unter-Types der FullAuditResult-Definition

```typescript
// API-Services nutzen exakt die gleichen Types wie JSON/HTML
class SitemapService {
  async getUrls(sitemapUrl: string): Promise<SitemapResult> {
    // Gibt SitemapResult zurück (gleicher Type wie in FullAuditResult.sitemap)
    return {
...
class AccessibilityService {
  async analyzeUrl(url: string): Promise<AccessibilityResult> {
    // Gibt AccessibilityResult zurück (gleicher Type wie in PageAuditResult.accessibility)
    return {
      passed: true,
      errors: [],
      warnings: [],
...
class PerformanceService {
  async analyzeUrl(url: string): Promise<PerformanceResult> {
    // Gibt PerformanceResult zurück (gleicher Type wie in PageAuditResult.performance)
  }
}

class SEOService {
  async analyzeUrl(url: string): Promise<SEOResult> {
    // Gibt SEOResult zurück (gleicher Type wie in PageAuditResult.seo)
  }
}
```

#### 3.2 API-Endpunkte für Electron Integration
```typescript
// API-Routen für modulare Abfragen
app.get('/api/sitemap/:domain', sitemapService.getUrls);
app.post('/api/accessibility/analyze', accessibilityService.analyzeUrl);
app.post('/api/performance/analyze', performanceService.analyzeUrl);
app.post('/api/seo/analyze', seoService.analyzeUrl);
```

#### 3.3 Selbst-dokumentierende API (minimaler Overhead)
**Ziel:** Auto-Generation der API-Dokumentation aus TypeScript Types

```typescript
// Swagger UI (bereits installiert: swagger-ui-express)
app.use('/api-docs', swaggerUi.serve, swaggerUi.setup(autoGeneratedSpec));

// Self-Introspection Endpoint
app.get('/api/schema', (req, res) => {
  res.json({
    types: generateFromTypeScript(['FullAuditResult', 'SitemapResult', 'AccessibilityResult']),
    endpoints: autoDiscoverEndpoints(),
    version: '2.0.0'
  });
});

// TypeScript → OpenAPI Schema Auto-Generation
const schemas = {
  SitemapResult: generateSchemaFromType(SitemapResult),
  AccessibilityResult: generateSchemaFromType(AccessibilityResult),
  PerformanceResult: generateSchemaFromType(PerformanceResult)
};
```

### Phase 4: Performance & Wartbarkeit (Priorität: NIEDRIG)
...
### Sprint 3: API-Modularisierung (Woche 5-7)
- [ ] Modulare Audit-Services erstellen (SitemapService, AccessibilityService, etc.)
- [ ] API-Endpunkte für Electron-Integration implementieren
- [ ] Auto-Schema Generation aus TypeScript Types
- [ ] Swagger UI Setup auf /api-docs
- [ ] Self-Introspection Endpoint (/api/schema)
- [ ] Einzelseiten-Audit-Funktionalität extrahieren

### Sprint 4: Performance (Woche 8-9)
